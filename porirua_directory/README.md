...
**My list:** Session-only favorites (`sessionStorage`, key `porirua-directory-favorites`) — each result card has **Add to your list** or **Remove** (with a trash icon; the button’s accessible name still includes the organisation or service name). When contact details exist, the card footer shows a **website** link (domain or social label) on the left and a **phone** link (icon + number) on the right. Open **My list** in the top nav or `#mylist` to review saved places and organisations. **Print list** uses the browser print dialog with a simplified layout (nav and crisis footer hidden; footer contact links remain, icons hidden). The UI states that nothing is stored on a server and the list clears when the tab closes.
