...
**Not in this slice:** Kubernetes manifests, Directus, and the weekly CronJob. Deployment needs (for the gated prod-tenant task): Postgres + PVC, `DATABASE_URL` as a Sealed Secret, optional `CATALOG_CURRENT_TTL_MS` (default 30000), the existing nginx Deployment, a second Deployment/Service for `porirua-directory-api` (port 3000), and Traefik path routing so `directory.bsky.nz/api` goes to the API while `/` stays on nginx. Editors publishing a snapshot do not need anyone to roll the API pod.
