...
**Do not:** implement Payload in this document; merge `feature/decision-record-and-p72`; touch `clusters/prod/**`; fork a second catalog.

Base branch for this inventory: `feature/editor-module-review-ota` (tip `76c0abb` when written). Public finder, Postgres, catalog API, and FSD stay as they are. Payload is a new editor surface + auth that must call the **same write path**.

---

## ADRs are not on the base branch

`docs/decisions/` **does not exist** on `feature/editor-module-review-ota`. All 23 files live on the **unmerged** branch `feature/decision-record-and-p72` (commit `a59b195`, one commit ahead of this tip).

Do **not** merge that branch from a Payload child. Read records with:

```bash
git ls-tree --name-only feature/decision-record-and-p72 docs/decisions/
git show feature/decision-record-and-p72:docs/decisions/018-undo-publish-version-guard.md
```

The same branch also has `docs/architecture/porirua-directory-deployment.md` (live services snapshot). That file is also absent from the base branch.

| ADR | Decision (one line) | Payload must honour |
|-----|---------------------|---------------------|
| 003 | Immutable `catalog_snapshots` + TTL pointer | Publish flips `is_current`; do not rebuild the envelope on each public request |
| 004 | Cloudflare purge on publish / rollback / undo-publish | Stay on the existing purge path; never set `CATALOG_SKIP_PURGE` on a tenant |
| 007 | Sidecar is unauthenticated; NetworkPolicy is the boundary | Port an authorizing proxy **before** widening the policy (see [Auth gate](#auth-gate)) |
| 008 | Custom Directory module, not stock Content | First login lands on Directory-shaped home, not collections |
| 009 | Sheet retired for the directory | Do not write directory listings back to the Connections Map sheet |
| 010 | Archive not delete; creates skip Review; near-name warn | Listings create never inserts `review_queue_items` |
| 013 | Status-write rules (changed stays published; approve respects hide; reject-new hides) | Call `approve-review.mjs`; do not invent status flips |
| 018 | Undo publish is version-checked | `POST /undo-publish` with `expectedVersion`; 24h or next publish |
| 019 | Three-way lock / sticky curation | Diff vs `raw_import`; save upserts `overrides` patch rows |
| 022 | Inbox is `review_queue_items`, not the `pending_review` view | Do not query or expose that view |
...
Anyone who can open `operations:8790` can publish the catalog, hide listings, approve FSD rows, and rewrite `raw_import`. That is the design (ADR 007), not a gap to paper over.

### Layer 3 — NetworkPolicy

Blackbox `clusters/dev/tenants/porirua-directory/networkpolicy-operations.yaml`: policy **`operations-from-directus-only`** admits ingress to `:8790` only from pods labelled `app: directus`. Operations is also patched **out of** the tenant-wide same-namespace and Traefik allow lists.
...
If the policy is widened first, the catalog becomes publishable by **anything that can reach the pod**. Do not add a shared secret “later”. Do not let the browser call the sidecar. Do not give the sidecar an Ingress.
...
|--------|------|-------------------|------------|
| GET | `/listings/name-matches` | `listings.nameMatches` → `lib/name-match.mjs` | Near-name check (ADR 010) |
...
| POST | `/undo-publish` | `undo-publish.undoPublish` + purge | Undo last publish (ADR 018) |
...
| Undo publish | `POST /undo-publish` | `expectedVersion` guard (ADR 018); `canUndoPublish` from last `catalog_publish_events` row + 24h; purge on rollback |
...
Inbox: `GET /queue` → `review_queue_items` (ADR 022). Community creates never appear. Kinds and buttons:
...
Reject of `new` hides the row (ADR 013). Reject of `changed` restores from `raw_import` and does not write `status`. Accept of `removed` is not used — take-off is `/hide`. `approveReviewItem` still archives if kind is `removed` (legacy); the UI must call `/hide` / `/keep-community` instead.
...
`POST /publish` calls `publishCatalog` (new snapshot, flip `is_current`, Cloudflare purge). A failed purge is a failed publish (ADR 004). It also `DELETE FROM editor_undo` — Review-decision undo is gone after Publish.
...
| Three-way lock (ADR 019) | `raw_import` + open `patch` overrides; Ora Toa (`fsd-2964`) is the live case |
| Hide lock | Open `overrides.action = hide` |
| Community-owned lock | Open `overrides.action = community_owned` |
| Name-check (ADR 010) | Warn on near duplicates including archived |
...
| Listings | Listings table + ADR 010 | Call listings routes; name-check; no Review enqueue |
| Review | Review table + ADR 019 / 022 | Government queue only; equal removal actions; defer / keep-community / review-undo |
| Publish / undo | Status band + ADR 003 / 004 / 018 | `publishStatus` fields; version guard; one publisher kill-switch |
