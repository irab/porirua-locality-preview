...
The runner keeps **one pending** `review_queue_items` row per entity+kind (refresh `proposed` in place). A `geocode_flag` already accepted or rejected for the same code is not raised again. Editors approve through **`approveReviewItem`** in `scripts/approve-review.mjs` (Directus imports the same export). Approve overlays payload keys onto the existing `raw_import` then fills missing fingerprint keys; a partial edit-and-approve must not blank `url` or `categories`. Reject never publishes a `hidden` row. Approve does not store an actor.
